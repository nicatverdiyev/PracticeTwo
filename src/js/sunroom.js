...
`
  ...
  // -----FİLTERED FUNCTİON----START---
    const searchElem = document.getElementById("searchProduc");
    searchElem.addEventListener("keyup", ()=>{
      let filteredProduc = products.filter(item=> item.title.toLowerCase().includes(searchElem.value.toLocaleLowerCase()))

      productsElem.innerHTML= "";
      filteredProduc.forEach(item => productsElem.innerHTML+=`
